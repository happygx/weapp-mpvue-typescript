<template>
  <div class="question-wrap">
    <van-cell-group :border="false">
      ...
      <van-cell title="问题级别" title-width="90px" :border="false">
        <p class="tl">
      ...
      <van-cell v-if="devices.length > 0 && !isView" :border="false" title=" " title-width="90px">
        <van-tag
          v-for="(item, index) in devices"
          :key="index"
          class="fl mr10 mb5"
      ...
      <van-cell v-if="!textareaShow" title="处理结果" title-width="90px" :border="false">
        <p class="tl" :class="{ placeholder: suggest === '' }">
          {{ suggest === '' ? '请输入处理结果' : suggest }}
        </p>
      </van-cell>
      <van-cell v-if="!isView" :border="false" title="上传附件" title-width="90px">
        <van-uploader
          class="fl"
          accept="media"
          preview-size="55px"
