 ...
 * @LastEditTime: 2020-12-16 09:21:05
 * @LastEditors: happy
-->
<template>
  <div class="information-wrap">
    <van-cell title="用户名" :value="info.username" />
    <van-cell title="姓名" :value="info.name" is-link @click="showPopup('name')" />
    <van-cell title="性别" :value="getGender" is-link @click="showPopup('gender')" />
    <van-cell title="手机号" :value="info.phone" is-link @click="showPopup('phone')" />
    <van-popup closeable position="bottom" :show="isShow" @close="closePopup">
      <div class="mt30 content clearfix">
        ...
        <template v-else-if="type === 'gender'">
          <van-radio-group class="radio-group" :value="gender" @change="gender = $event.mp.detail">
        ...
        <template v-else>
          <van-cell-group>
            ...
            >
              <van-button slot="button" size="small" type="primary" @click="isSend && sendCode()">
                {{ sendText }}
              </van-button>
            </van-field>
          </van-cell-group>
